...
//navHover
var $navAct = $('.nav-act');
function goback(left){$navAct.css('left',String(left+'px'));
$navAct.css('transition','linear 1s');$navAct.css('transform','skew(0deg)');}
...
$(document).ready(function(){
    ...
    //navHover
    var $oldML = 0;
    $(".navul li").mouseenter(function(e){
    	
        $oldML = $navAct.css('left');
       
        $navAct.css('left',String(($(this).index())*125)+'px');
//      console.log($(".nav-act").width());
            console.log($navAct.css('left'));
        	console.log(e.clientX)
        	console.log($(".navul").offset().left+60)
        if(parseInt($navAct.css('left'))<e.clientX-($(".navul").offset().left+60)){
